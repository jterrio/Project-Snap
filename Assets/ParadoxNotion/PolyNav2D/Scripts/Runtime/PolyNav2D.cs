		...
		void CreatePolyMap(bool generateMaster){
			...
			for (int i = 0; i < navObstacles.Count; i++){
				...
				for (var p = 0; p < obstacle.GetPathCount(); p++){
					var transformed = TransformPoints(obstacle.GetPathPoints(p), obstacle.transform);
					var inflated = InflatePolygon(transformed, rad);
			...
			if (generateMaster){
				...
				if (masterCollider is PolygonCollider2D){
					...
					for (int i = 0; i < polyCollider.pathCount; ++i){
						...
						var transformed = TransformPoints(points, polyCollider.transform);
				...
				foreach(var obstacle in navObstacles){
					if (obstacle != null){
						for (var i = 0; i < obstacle.GetPathCount(); i++){
							DebugDrawPolygon(TransformPoints(obstacle.GetPathPoints(i), obstacle.transform), new Color(1, 0.7f, 0.7f));
						}
					}
				}
